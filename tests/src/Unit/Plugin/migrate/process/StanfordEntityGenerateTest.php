...
use Drupal\stanford_migrate\Plugin\migrate\process\StanfordEntityGenerate;
...
/**
 * Class StanfordEntityGenerateTest.
 *
 * @group stanford_migration
 * @coversDefaultClass \Drupal\stanford_migrate\Plugin\migrate\process\StanfordEntityGenerate
 */
class StanfordEntityGenerateTest extends UnitTestCase {
  ...
  public function testTranform() {
    ...
    $definition = [];
    $plugin = StanfordEntityGenerate::create($this->container, $configuration, 'entity_generate_no_lookup', $definition, $migration);

    $migrate_executable = $this->createMock(MigrateExecutable::class);
    $row = $this->createMock(Row::class);

    $this->assertInstanceOf(StanfordEntityGenerate::class, $plugin);
